...
async function all() {
  if (stop == 0) {
    await Promise.all([
	  ...
	  //JingDongInStore(stop), //京东进店有礼
	  JDMagicCube(stop), //京东小魔方
	  JingDongLottery(stop,'JDLottery'), //京东刮奖
	  JingDongLottery(stop,'JDLottery2'), //京东金融抽奖	
	  
      JDUserSign(stop,'JDGStore'), //京东超市
      JDUserSign(stop,'JDPet'), //京东宠物馆
      JDUserSign(stop,'JDBook'), //京东图书
      JDUserSign(stop,'JDShand'), //京东拍拍二手
      JDUserSign(stop,'JDMakeup'), //京东美妆馆
      JDUserSign(stop,'JDWomen'), //京东女装馆
      JDUserSign(stop,'JDVege'), //京东菜场
      JDUserSign(stop,'JDFood'), //京东美食馆
      JDUserSign(stop,'JDClean'), //京东清洁馆
      JDUserSign(stop,'JDCare'), //京东个人护理馆
      JDUserSign(stop,'JDJewels'), //京东珠宝馆
	  JDUserSign(stop,'JDClocks'), //京东钟表馆	 
	  JDUserSign(stop,'JDSleep') //京东失眠
	...
	//await JingDongInStore(stop); //京东进店有礼
		
	await JingDongLottery(stop,'JDLottery'); //京东刮奖	
	await JingDongLottery(stop,'JDLottery2'); //京东刮奖2	
	
	await JDUserSign(stop,'JDVege'); //京东菜场	
    await JDUserSign(stop,'JDGStore'); //京东超市
    await JDUserSign(stop,'JDPet'); //京东宠物馆
    await JDUserSign(stop,'JDBook'); //京东图书
    await JDUserSign(stop,'JDShand'); //京东拍拍二手
    await JDUserSign(stop,'JDMakeup'); //京东美妆馆
    await JDUserSign(stop,'JDWomen'); //京东女装馆
    await JDUserSign(stop,'JDJewels'); //京东珠宝馆
    await JDUserSign(stop,'JDFood'); //京东美食馆
	await JDUserSign(stop,'JDSleep'); //京东失眠
    await JDUserSign(stop,'JDClean'); //京东清洁馆
    await JDUserSign(stop,'JDCare'); //京东个人护理馆
	await JDUserSign(stop,'JDClocks'); //京东钟表馆	
...
function JingDongInStore(s) {
  return new Promise(resolve => {
    setTimeout(() => {
      const login = {
        url: 'https://api.m.jd.com/client.action?functionId=queryTaskIndex',
        headers: {
          "Content-Type": "application/x-www-form-urlencoded",
          Cookie: KEY,
        },
        body: "adid=25AB663D-D6DC-4140-957C-FA39B305DA55&area=13_1000_4277_63096&body=%7B%7D&build=167283&client=apple&clientVersion=9.0.4&d_brand=apple&d_model=iPhone10%2C2&eid=YINOPXQH3B27U2V6CLVWGNHFGUZAANV35MJOUK75XIXZZXKR4DXP4ESU5TUAHEUPYLAIOXB5YYIO4TFL7245ASUZX2R5X6WPTTMLLLJOT2CZYO6GPVVA&isBackground=N&joycious=120&lang=zh_CN&networkType=4g&networklibtype=JDNetworkBaseAF&openudid=e52a4caaec5f73d41afe51e92ced027a48e63a83&osVersion=13.6&partner=apple&rfs=0000&scope=11&screen=1242%2A2208&sign=747e08d5fb2d31662e51686ecdcf372f&st=1594415825855&sv=100&uts=0f31TVRjBSt1ATQvFwnjIfpu%2Bz85BCDJ0PDFmdQ/lAhpOY/1X6IHGRfTLDm674SrKFhWGra%2BJLCX/fITFJoOFQbrNaHqnUCtE050%2B1UAyDnNKLCzyP6gZI9ejT6krpfOzKhinq6ivKAWFFkORbkfWUJE6jWSNr3t6Hh/5GoWY6UnwOjWQX80%2BegiMii%2BFibGjH9frio03xDnWlSpXl0lGw%3D%3D"
      };
      $nobyda.post(login,async function(error, response, data) {
        try {
          if (error) {
            merge.JDInStore.notify = "京东商城-进店获取ID: 登录接口请求失败 ‼️‼️"
            merge.JDInStore.fail = 1
          } else {
			var cc = JSON.parse(data);			
            const Details = LogDetails ? "response:\n" + data : '';
            if (data.match(/\"code\":\"0/)) {
              console.log("\n" + "京东商城-进店获取ID登录成功 " + Details);
			  
		      var ja = cc.data.taskList;
			  for(var i in ja){
				  console.log("\n" + ja[i].taskId);
				  await DoJingDongInStore(200,ja[i].taskId);
			  }
			  data = null;cc = null;ja = null;
            } else {
              console.log("\n" + "京东商城-进店获取ID登录失败 " + Details)
              if (data.match(/\"code\":\"1/)) {
                merge.JDInStore.notify = "京东商城-进店获取ID: 失败, 原因:" + cc.echo + "‼️"
                merge.JDInStore.fail = 1
              } else 
			  if (data.match(/\"code\":\"3\"/)) {
                merge.JDInStore.notify = "京东商城-进店: 失败, 原因: Cookie失效‼️" ;
                merge.JDInStore.fail = 1				
              } else {
                merge.JDInStore.notify = "京东商城-进店获取ID: 登录接口需修正 ‼️‼️"
                merge.JDInStore.fail = 1
              }
            }
          }
          ...
          $nobyda.notify("京东金融-京豆登录获取ID " + eor.name + "‼️", JSON.stringify(eor), eor.message)
          resolve()
        }
      })
    }, s)
...
function DoJingDongInStore(s,taskid) {
  return new Promise(resolve => {
    setTimeout(() => {
      const login = {
        url: 'https://api.m.jd.com/client.action?functionId=takeTask',
        headers: {
          "Content-Type": "application/x-www-form-urlencoded",
          Cookie: KEY,
        },
        body: "adid=25AB663D-D6DC-4140-957C-FA39B305DA55&area=13_1000_4277_63096&body=%7B%22taskId%22%3A%22" + taskid + "%22%7D&build=167283&client=apple&clientVersion=9.0.4&d_brand=apple&d_model=iPhone10%2C2&eid=YINOPXQH3B27U2V6CLVWGNHFGUZAANV35MJOUK75XIXZZXKR4DXP4ESU5TUAHEUPYLAIOXB5YYIO4TFL7245ASUZX2R5X6WPTTMLLLJOT2CZYO6GPVVA&isBackground=N&joycious=120&lang=zh_CN&networkType=4g&networklibtype=JDNetworkBaseAF&openudid=e52a4caaec5f73d41afe51e92ced027a48e63a83&osVersion=13.6&partner=apple&rfs=0000&scope=11&screen=1242%2A2208&st=1594415828239&sv=101&uts=0f31TVRjBSt1ATQvFwnjIfpu%2Bz85BCDJ0PDFmdQ/lAhpOY/1X6IHGRfTLDm674SrKFhWGra%2BJLCX/fITFJoOFQbrNaHqnUCtE050%2B1UAyDnNKLCzyP6gZI9ejT6krpfOzKhinq6ivKAWFFkORbkfWUJE6jWSNr3t6Hh/5GoWY6UnwOjWQX80%2BegiMii%2BFibGjH9frio03xDnWlSpXl0lGw%3D%3D"
		};
      $nobyda.post(login, function(error, response, data) {
        try {
          if (error) {
			merge.JDInStore.notify += merge.JDInStore.notify ? "\n京东商城-进店: 登录接口请求失败 ‼️‼️ (多次)" : "京东商城-进店: 登录接口请求失败 ‼️‼️";
            merge.JDInStore.fail += 1
          } else {
			const cc = JSON.parse(data);			
            //const Details = LogDetails ? "response:\n" + data : '';
            ...
            if (data.match(/\"code\":\"0/)) {
			  if (data.match(/(\d+)京豆/)) {
                console.log("\n" + "京东商城-进店成功 " + Details);
			    merge.JDInStore.notify += merge.JDInStore.notify ? "\n京东商城-进店: 成功, 明细: " + data.match(/(\d+)京豆/)[1] + "京豆 🐶 (多次)" : "京东商城-进店: 成功, 明细: " + data.match(/(\d+)京豆/)[1] + "京豆 🐶"
                merge.JDInStore.success += 1
                merge.JDInStore.bean += Number(data.match(/(\d+)京豆/)[1]);
			  } else {
				console.log("\n" + "京东商城-进店失败 " + Details)
				merge.JDInStore.notify += merge.JDInStore.notify ? "\n京东商城-进店: 失败, 原因: 已领过(多次) ⚠️" : "京东商城-进店: 失败, 原因: 已领过 ⚠️" ;
                merge.JDInStore.fail += 1
			  }	
            } else {
              console.log("\n" + "京东商城-进店失败 " + Details)
              if (data.match(/\"code\":\"1/)) {
                merge.JDInStore.notify += merge.JDInStore.notify ? "\n京东商城-进店: 失败, 原因:" + cc.echo + "(多次) ⚠️" : "京东商城-进店: 失败, 原因:" + cc.echo + " ⚠️" ;
                merge.JDInStore.fail += 1
              } else
			  if (data.match(/\"code\":\"3\"/)) {
                merge.JDInStore.notify += merge.JDInStore.notify ? "\n京东商城-进店: 失败, 原因: Cookie失效(多次)‼️" : "京东商城-进店: 失败, 原因: Cookie失效‼️" ;
                merge.JDInStore.fail += 1				
              } else {
                merge.JDInStore.notify += merge.JDInStore.notify ? "京东商城-进店: 失败, 原因:未知 (多次) ⚠️" : "京东商城-进店: 失败, 原因:未知 ⚠️";
                merge.JDInStore.fail += 1
              }
            }
          }
          ...
          $nobyda.notify("京东商城-进店" + eor.name + "‼️", JSON.stringify(eor), eor.message)
          resolve()
        }
      })
    }, s)
...
function JingDongLottery(s,key) {
  return new Promise(resolve => {
    setTimeout(() => {
      var login = {
      ...
      $nobyda.post(login, async function (error, response, data) {
          try {
            if (error) {
              merge[key].notify = merge[key].nickname + ": 接口请求失败 ‼️‼️"
              merge[key].fail = 1
            } else {
              const c = JSON.parse(data)
              const Details = LogDetails ? "response:\n" + data : '';
			  //const Details = "response:\n" + data ;
			  //console.log("\n" + merge[key].nickname  + Details)
              if (data.match(/\"winner\":\"true/)) {
                console.log("\n" + merge[key].nickname + "抽奖成功 " + Details)
                if (data.match(/(\d+)京豆/)) {
                  merge[key].notify = merge[key].nickname +": 成功, 明细: " + data.match(/(\d+)京豆/)[1] + "京豆 🐶"
                  merge[key].success = 1
                  merge[key].bean = data.match(/(\d+)京豆/)[1];
                } else {
				  merge[key].notify = `${merge[key].nickname}: 成功, 明细:${c.prizeName} 无京豆 🐶`
                  merge[key].success = 1
                }
              } else {
                if (data.match(/(擦肩而过|没抽中)/)) {
				  console.log("\n" + merge[key].nickname + "签到成功" + Details)	
                  merge[key].notify = `${merge[key].nickname}: 成功, 明细: 无京豆 🐶`
                  merge[key].success = 1
                } else {
				  console.log("\n" + merge[key].nickname + "签到失败 " + Details)	
                  if (data.match(/(用完|再来)/)) {
                    merge[key].notify = `${merge[key].nickname}: 失败, 原因: 已抽过 ⚠️`
                    merge[key].fail = 1
                  } else
                  if (data.match(/\"code\":\"3\"/)) {
                    merge[key].notify = merge[key].nickname + ": 失败, 原因: Cookie失效‼️"
                    merge[key].fail = 1
                  } else
                  if (data.match(/\"promptMsg\"/)) {
                    merge[key].notify = `${merge[key].nickname}: 失败, 原因: ${c.promptMsg} ⚠️`
                    merge[key].fail = 1
                  } else {
                    merge[key].notify = merge[key].nickname + ": 原因: 未知 ⚠️‼"
                    merge[key].fail = 1
                  }
                }
              }
            ...
            $nobyda.notify(merge[key].nickname + eor.name + "‼️", JSON.stringify(eor), eor.message)
            resolve()
          }
        })
    }, s)
...
function JingRongSteel(s) {
  return new Promise(resolve => {
    setTimeout(() => {
      const JRSUrl = {
        url: 'https://ms.jr.jd.com/gw/generic/gry/h5/m/signIn1?_=1594236220240',
        headers: {
          "Content-Type": "application/x-www-form-urlencoded",
          Cookie: KEY,
        },
            ...
            if (data.match(/\"resultCode\":0/)) {
              if (data.match(/\"count\":\d+/)) {
                ...
                if (data.match(/已领取/)) {
                  ...
                  merge.JRDSign.notify = "京东金融-双签: 失败, 原因: 未完成双签 ⚠️"
                } else {
...
function JDUserSign(s,key) {
  return new Promise(resolve => {
    setTimeout(() => {
	  ...
	  GetJdSignBody(key,function(body){
	  if (body == "undefined")
      {
          merge[key].notify = `${merge[key].nickname}: 获取body失败 ‼️‼️`
          merge[key].fail = 1
      }  else
      {
	  JDUrl.body = `body=${body}`
	  }  
	  
      $nobyda.post(JDUrl, function(error, response, data) {
        try {
          if (error) {
            merge[key].notify = `${merge[key].nickname}: 签到接口请求失败 ‼️‼️`
            merge[key].fail = 1
          } else {
            const Details = LogDetails ? "response:\n" + data : '';
            const cc = JSON.parse(data)
            if (data.match(/签到成功/)) {
              ...
              if (data.match(/(\"text\":\"\d+京豆\")/)) {
                ...
                merge[key].notify = `${merge[key].nickname}: 成功, 明细: ${beanQuantity}京豆 🐶`
              ...
              if (data.match(/(已签到|已领取)/)) {
                merge[key].notify = `${merge[key].nickname}: 失败, 原因: 已签过 ⚠️`
                merge[key].fail = 1
              } else {
                if (data.match(/(不存在|已结束|未开始)/)) {
                  merge[key].notify = `${merge[key].nickname}: 失败, 原因: 活动已结束 ⚠️`
                  merge[key].fail = 1
                } else {
                  if (cc.code == 3) {
                    merge[key].notify = `${merge[key].nickname}: 失败, 原因: Cookie失效‼️`
                    ...
                    merge[key].notify = `${merge[key].nickname}: 失败, 原因: 认证失败 ⚠️`
                    merge[key].fail = 1
                  } else {
                    merge[key].notify = `${merge[key].nickname}: 失败, 原因: 未知 ⚠️`
                    merge[key].fail = 1
                  }
                }
              }
...
function initial() {
  ...
  merge.JDSleep.nickname = '京东商城-失眠';
  merge.JDSleep.url = 'https://pro.m.jd.com/mall/active/xK148m4kWj5hBcTPuJUNNXH3AkJ/index.html';

  merge.JDGStore.nickname = '京东商城-超市';
  merge.JDGStore.url = 'https://pro.m.jd.com/mall/active/aNCM6yrzD6qp1Vvh5YTzeJtk7cM/index.html';

  merge.JDPet.nickname = '京东商城-宠物';
  merge.JDPet.url='https://pro.m.jd.com/mall/active/37ta5sh5ocrMZF3Fz5UMJbTsL42/index.html';
  
  merge.JDBook.nickname = '京东商城-图书';
  merge.JDBook.url = 'https://pro.m.jd.com/mall/active/3SC6rw5iBg66qrXPGmZMqFDwcyXi/index.html';
  
  merge.JDShand.nickname = '京东拍拍-二手';
  merge.JDShand.url = 'https://pro.m.jd.com/mall/active/3S28janPLYmtFxypu37AYAGgivfp/index.html';

  merge.JDWomen.nickname = '京东商城-女装';
  merge.JDWomen.url = 'https://pro.m.jd.com/mall/active/DpSh7ma8JV7QAxSE2gJNro8Q2h9/index.html'; 
  
  merge.JDMakeup.nickname = '京东商城-美妆';
  merge.JDMakeup.url = 'https://pro.m.jd.com/mall/active/2smCxzLNuam5L14zNJHYu43ovbAP/index.html';
   
  merge.JDClean.nickname = '京东商城-清洁';
  merge.JDClean.url = 'https://pro.m.jd.com/mall/active/2Tjm6ay1ZbZ3v7UbriTj6kHy9dn6/index.html';
  
  merge.JDCare.nickname = '京东商城-个护';
  merge.JDCare.url = 'https://pro.m.jd.com/mall/active/NJ1kd1PJWhwvhtim73VPsD1HwY3/index.html';
  
  merge.JDFood.nickname = '京东商城-美食';
  merge.JDFood.url = 'https://pro.m.jd.com/mall/active/4PzvVmLSBq5K63oq4oxKcDtFtzJo/index.html';
  
  merge.JDJewels.nickname = '京东商城-珠宝';
  merge.JDJewels.url = 'https://pro.m.jd.com/mall/active/zHUHpTHNTaztSRfNBFNVZscyFZU/index.html';
  
  merge.JDVege.nickname = '京东商城-菜场';
  merge.JDVege.url = 'https://pro.m.jd.com/mall/active/Wcu2LVCFMkBP3HraRvb7pgSpt64/index.html';
  
  merge.JDClocks.nickname = '京东商城-钟表';
  merge.JDClocks.url = 'https://pro.m.jd.com/mall/active/2BcJPCVVzMEtMUynXkPscCSsx68W/index.html';
  
  merge.JDLottery.nickname = '京东商城-刮奖';
  merge.JDLottery.url = 'https://prodev.m.jd.com/mall/active/Pz7vAj2rsz1aEmZgAAk1fG9fEAn/index.html?from=qdy-icon';
  merge.JDLottery.body = 'body=%7B%22lotteryCode%22%3A%22b8b5edd5-2b42-4743-ae0d-51afd4226882%22%2C%22authType%22%3A%222%22%2C%22riskParam%22%3A%7B%22platform%22%3A%223%22%2C%22orgType%22%3A%222%22%2C%22openId%22%3A%22-1%22%2C%22pageClickKey%22%3A%22Babel_ScratchTicket%22%2C%22eid%22%3A%224NCPNCJW746YAZQW6X7FBOXQW5XSZU2QLEKSFJARZS7ZP2ZDYWDRC2NG3WCBI6UZKQ54W5VKU7QAPA2IX7K4BZ24LE%22%2C%22fp%22%3A%22dfa8f4287a92af72d3fb069cfa5f4226%22%2C%22shshshfp%22%3A%22256afe2849d9079bae0f374222f7f209%22%2C%22shshshfpa%22%3A%223b42c257-c841-d575-572f-f35472b03b3d-1591861825%22%2C%22shshshfpb%22%3A%22gfWqHm1%2FTbKoJASyOMAsSxA%3D%3D%22%2C%22childActivityUrl%22%3A%22https%3A%2F%2Fprodev.m.jd.com%2Fmall%2Factive%2FPz7vAj2rsz1aEmZgAAk1fG9fEAn%2Findex.html%3Ffrom%3Dqdy-icon%22%7D%2C%22source%22%3A%22babel%22%2C%22uniqueCode%22%3A%22%22%2C%22mitemAddrId%22%3A%22%22%2C%22geo%22%3A%7B%22lng%22%3A%22%22%2C%22lat%22%3A%22%22%7D%2C%22addressId%22%3A%22%22%2C%22posLng%22%3A%22%22%2C%22posLat%22%3A%22%22%2C%22focus%22%3A%22%22%2C%22innerAnchor%22%3A%22%22%2C%22cv%22%3A%222.0%22%7D&client=wh5'

  merge.JDLottery2.nickname = '京东金融-抽奖';
  merge.JDLottery2.url = 'https://prodev.m.jd.com/mall/active/4U7xcDsnuNEm4D3DygUM5Dafwb2t/index.html';
  merge.JDLottery2.body = 'body=%7B%22lotteryCode%22%3A%22745a2139-d8da-4304-b6bd-e77d924d42e9%22%2C%22authType%22%3A%222%22%2C%22riskParam%22%3A%7B%22platform%22%3A%223%22%2C%22orgType%22%3A%222%22%2C%22openId%22%3A%22-1%22%2C%22pageClickKey%22%3A%22Babel_WheelSurf%22%2C%22eid%22%3A%224NCPNCJW746YAZQW6X7FBOXQW5XSZU2QLEKSFJARZS7ZP2ZDYWDRC2NG3WCBI6UZKQ54W5VKU7QAPA2IX7K4BZ24LE%22%2C%22fp%22%3A%2213d0dce7cd91ac88b5a9c19c28a5149b%22%2C%22shshshfp%22%3A%22d107f5cfbd69123c6e0ae3b99fce5152%22%2C%22shshshfpa%22%3A%223b42c257-c841-d575-572f-f35472b03b3d-1591861825%22%2C%22shshshfpb%22%3A%22gfWqHm1%2FTbKoJASyOMAsSxA%3D%3D%22%2C%22childActivityUrl%22%3A%22https%3A%2F%2Fprodev.m.jd.com%2Fmall%2Factive%2F4U7xcDsnuNEm4D3DygUM5Dafwb2t%2Findex.html%22%7D%2C%22mitemAddrId%22%3A%22%22%2C%22geo%22%3A%7B%22lng%22%3A%22%22%2C%22lat%22%3A%22%22%7D%2C%22addressId%22%3A%22%22%2C%22posLng%22%3A%22%22%2C%22posLat%22%3A%22%22%2C%22focus%22%3A%22%22%2C%22innerAnchor%22%3A%22%22%2C%22cv%22%3A%222.0%22%7D&client=wh5'
}
...
function GetJdSignBody(key, callback)
{
 let JDUrl = {
        url: merge[key].url,
        headers: {
      ...
      try {
        if (!error) 
		{
          let body = data.match(/{\\"enActK\\":\\".+?}/);
		  JDUrl = null;data = null; //尝试释放来解决QX签到异常退出
		  if (body == '')
		  {
		     callback ("undefined");
		  } else
		  {
		    body = `{"params":"${body}","riskParam":{"platform":"3","orgType":"2","openId":"-1","pageClickKey":"Babel_Sign","eid":"KLAZV3VLA44LYG7H3DDBQOIP4QHT5RKZNZHW3VMPNMMK2EV5JQTJPOCXYFJQIJ7QK7YX7EIP3YOENKOSIKYLK5TDXQ","fp":"6ac815a9da4b74d00e859782b86391ec","shshshfp":"1f65983b416a367aafd8b79387d4ebb6","shshshfpa":"07158aca-73cf-318b-c731-1428a3453677-1593044801","shshshfpb":"pYbU62avHFvpNOIrbqmo99g==","childActivityUrl":"${merge[key].url}"},"mitemAddrId":"","geo":{"lng":"","lat":""},"addressId":"","posLng":"","posLat":"","focus":"","innerAnchor":"","cv":"2.0"}`
            //console.log(`\n${body}`);				
			callback (urlencode(body)+'&screen=750*1334&client=wh5&clientVersion=1.0.0&sid=');
		  }	
        } else 
		{
          merge[key].notify = `${merge[key].nickname}: 获取body失败 ‼️‼️`
          merge[key].fail = 1
          console.log(`\n${merge[key].nickname}：获取签到body失败`)
        }

      } catch (eor) {
        $nobyda.notify(`${merge[key].nickname}${eor.name} ‼️`, JSON.stringify(eor), eor.message)
        merge[key].notify = `${merge[key].nickname}: ${eor.message}‼️‼️`
          merge[key].fail = 1
      }
    })
}
